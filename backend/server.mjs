...
function normalizeContent(value){
  ...
  return {
    ...
    jobs:value.jobs.map(item=>({id:cleanText(item.id,100),slug:cleanText(item.slug,100),title:cleanText(item.title,200),department:cleanText(item.department,120),category:['sales','operations','tech'].includes(item.category)?item.category:'operations',employmentType:cleanText(item.employmentType,80),location:cleanText(item.location,150),workSetup:status(item.workSetup,['onsite','hybrid','remote'],'onsite'),priority:['urgent','active','open'].includes(item.priority)?item.priority:'open',summary:cleanText(item.summary,5000),description:cleanText(item.description,10000),responsibilities:Array.isArray(item.responsibilities)?item.responsibilities.slice(0,100).map(value=>cleanText(value,1000)).filter(Boolean):[],qualifications:Array.isArray(item.qualifications)?item.qualifications.slice(0,100).map(value=>cleanText(value,1000)).filter(Boolean):[],image:cleanUrl(item.image),imageMediaId:cleanMediaId(item.imageMediaId),status:status(item.status,['draft','open','closed','archived'],'open'),publishedAt:cleanText(item.publishedAt,100),closesAt:cleanText(item.closesAt,100)}))
  };
}
